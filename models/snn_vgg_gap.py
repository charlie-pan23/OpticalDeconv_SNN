"""Generalized Spiking VGG with Global Average Pooling.

Recommended for CIFAR10-DVS in the HIPSA evaluation pipeline. Compared with the
legacy SpikingVGG, this model removes the huge 16384->512 fully connected layer
and replaces it with global average pooling followed by a compact classifier.
This usually improves generalization on small event datasets and makes the model
more suitable for reproducible paper experiments.
"""
...
class SpikingVGGGAP(nn.Module):
    """VGG-style SNN with GAP classifier.
    ...
    def __init__(
        ...
        channels: Sequence[int] = (32, 64, 128, 128, 256, 256),
        hidden_dim: int = 256,
        tau: float = 2.0,
        ...
        super().__init__()
        if len(channels) != 6:
            raise ValueError("SpikingVGGGAP expects exactly 6 convolution channel values")

        c1, c2, c3, c4, c5, c6 = [int(c) for c in channels]
        ...
        self.conv6 = nn.Conv2d(c5, c6, 3, padding=1, bias=False)
        self.bn6 = bn(c6)
        ...
        self.fc1 = nn.Linear(c6, hidden_dim, bias=False)
